...
export class StudentService {
  constructor(private readonly httpService: HttpService, private readonly eventsGateway: EventsGateway) {}

  async readStudentData(fileName: string): Promise<any> {
    let data;
    let studentData: any = [];
    readXlsxFile(`./upload/${fileName}`).then(async (rows) => { // read uploaded excel file 
      ...
      rows.map(async (row, rIndex) => {
          data = {};
          if (rIndex !== 0) { // ignore heading row
              row.map((cell, cIndex) => {
      ...
      students.forEach((student: any) => {
        date = moments(student.dateOfBirth.toString(), 'x').format('YYYY-MM-DD');
  ...
  async saveStudentData(students: CreateStudentInput[]): Promise<Observable<AxiosResponse<any[], any>>> {
    ...
    const data = await lastValueFrom(
      this.httpService.post('http://data_store:3000/graphql', {
