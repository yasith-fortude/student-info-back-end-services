...
export class StudentController {
    ...
    @UseInterceptors(FileInterceptor('file', { // used multer to save upload file with a custom name & saving location (inorder to access again to the read file)
        storage: diskStorage({
            destination: './upload',
    ...
    async uploadSingle(@UploadedFile() file: Express.Multer.File): Promise<object> {
        try {
            await this.studentService.addToQueue(file); // use service layer method to add data to queue

            return { success: true, status: HttpStatus.CREATED, message: 'Upload success.' };
        } catch (error) {
